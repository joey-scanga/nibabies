...
from nibabies.interfaces.maths import TemporalMean
from nibabies.interfaces.resampling import (
    DistortionParameters,
    ReconstructFieldmap,
    ResampleSeries,
)
from nibabies.interfaces.warpkit import WarpkitMEDIC
from nibabies.utils.bids import extract_entities, collect_bold_part_files
...
def init_bold_fit_wf(
    ...
    * :py:func:`~nibabies.workflows.bold.outputs.init_ds_registration_wf`
    ...
    if warpkit_enabled:
        ...
        # try:
        #     tes_ms = float(metadata['EchoTime']) * 1000
        ...
        tes_ms = [
            float(layout.get_metadata(p).get('EchoTime', 0.0)) * 1000 for p in bold_series
        ]
        phase_tes_ms = [
...
def init_bold_boldref_wf(
    *,
    ...
    elif use_warpkit:  # fieldmap already in native space
        workflow.connect([
            (distortion_params, boldref_bold, [
                ('readout_time', 'ro_time'),
                ('pe_direction', 'pe_dir'),
            ]),
            (inputnode, boldref_bold, [('fieldmap', 'fieldmap')]),
        ])  # fmt:skip
